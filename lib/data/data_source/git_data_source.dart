...
class GitDataSource {
  ...
  Future<List<PostDto>> getDir(
      {required String owner, required String repo}) async {
    final response =
        await http.get(Uri.parse('$_baseUrl/repos/$owner/$repo/contents/'));
    try {
      final List body = jsonDecode(response.body);

      final result = body.map((e) => PostDto.fromJson(e)).toList();
      return result;
    } catch (e) {
      throw Exception('다시 입력');
    }
  }
  ...
  Future<PostDto> getPost(
    ...
    final response =
        await http.get(Uri.parse('$_baseUrl$owner/$repo/contents/$dir/$file'));
